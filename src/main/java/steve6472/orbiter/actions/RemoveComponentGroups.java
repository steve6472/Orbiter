 ...
 * Project: Orbiter <br>
 */
public record RemoveComponentGroups(OrCode condition, EntitySelection entitySelection, List<StringSource> groups) implements Action
{
    private static final Logger LOGGER = Log.getLogger(RemoveComponentGroups.class);
    public static final Codec<RemoveComponentGroups> CODEC = RecordCodecBuilder.create(instance -> instance.group(
    ...
    ).apply(instance, RemoveComponentGroups::new));
    ...
    public void execute(World world, Entity entity, OrlangEnv environment)
    {
        Components.BLUEPRINT_REFERENCE.ifPresent(entity, blueprintKey -> {
            ...
            for (StringSource groupSource : groups)
            {
                ...
                componentGroup.forEach((entry, _) -> entity.remove(entry.componentClass()));
            }
        });
    }
    ...
    {
        return ActionType.REMOVE_COMPONENT_GROUPS;
    }
}
