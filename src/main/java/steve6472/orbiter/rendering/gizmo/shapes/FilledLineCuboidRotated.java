 ...
 * Project: Orbiter <br>
 */
public record FilledLineCuboidRotated(Vector3f start, Vector3f end, int lineColor, int fillColor, float lineWidth, Quaternionf rotation) implements Gizmo
{
    public FilledLineCuboidRotated(Vector3fc center, float halfSize, int lineColor, int fillColor, float lineWidth, Quaternionf rotation)
    {
        this(new Vector3f(center).sub(halfSize, halfSize, halfSize), new Vector3f(center).add(halfSize, halfSize, halfSize), lineColor, fillColor, lineWidth, rotation);
    }

    public FilledLineCuboidRotated(Vector3fc center, float halfWidth, float halfHeight, float halfDepth, int lineColor, int fillColor, float lineWidth, Quaternionf rotation)
    {
        this(new Vector3f(center).sub(halfWidth, halfHeight, halfDepth), new Vector3f(center).add(halfWidth, halfHeight, halfDepth), lineColor, fillColor, lineWidth, rotation);
    }
    ...
    public void create(GizmoPrimitives primitives, AlphaMultiplier alpha)
    {
        ...
        if (rotation != null)
        {
            ...
            p0 = rotateAroundCenter(p0, center, rotation);
            p1 = rotateAroundCenter(p1, center, rotation);
            p2 = rotateAroundCenter(p2, center, rotation);
            p3 = rotateAroundCenter(p3, center, rotation);
            p4 = rotateAroundCenter(p4, center, rotation);
            p5 = rotateAroundCenter(p5, center, rotation);
            p6 = rotateAroundCenter(p6, center, rotation);
            p7 = rotateAroundCenter(p7, center, rotation);
        }
    ...
    private Vector3f rotateAroundCenter(Vector3f point, Vector3f center, Quaternionf rotation)
    {
