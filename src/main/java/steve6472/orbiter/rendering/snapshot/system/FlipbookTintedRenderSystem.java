 ...
 * Project: Orbiter <br>
 */
public class FlipbookTintedRenderSystem extends CommonRenderSystem
{
    ...
    public FlipbookTintedRenderSystem(MasterRenderer masterRenderer, ParticleMaterial material, Client client)
    {
        super(masterRenderer, material.flipbookPipeline(),
            CommonBuilder
                .create()
                .entryImage(atlasSampler())
                .entrySBO(OrbiterSBO.PARTICLE_FLIPBOOK_TINTED_ENTRIES.sizeof(), VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, VK_SHADER_STAGE_FRAGMENT_BIT)
    ...
    protected void render(FlightFrame flightFrame, FrameInfo frameInfo, MemoryStack stack)
    {
        ...
        List<FlipbookTintedParticlePair> list = currentRenderState.flipbookTintedParticles.get(material);
        ...
        int renderedCount = 0;
        for (FlipbookTintedParticlePair snapshotPair : list)
        {
            position.set(0, 0, 0);
            rotation.identity();

            FlipbookTintedParticleSnapshot previousSnapshot = snapshotPair.previous();
            FlipbookTintedParticleSnapshot currentSnapshot = snapshotPair.current();
    ...
    protected void updateData(FlightFrame flightFrame, FrameInfo frameInfo)
    {
        ...
        int size = list.size() * OrbiterSBO.PARTICLE_FLIPBOOK_TINTED_ENTRY.sizeof() * VERTEX_COUNT;
        ByteBuffer byteBuffer = buffer
            .getMappedMemory()
            .getByteBuffer(0, size);
        ...
        for (FlipbookTintedParticlePair flipbookParticlePair : list)
        {
            FlipbookTintedParticleSnapshot snapshot = flipbookParticlePair.current();
            ...
            byteBuffer.putFloat(snapshot.r).putFloat(snapshot.g).putFloat(snapshot.b).putFloat(snapshot.a);
        }
    }
    ...
    protected boolean shouldRender()
    {
        ...
        WorldRenderState currentRenderState = OrbiterApp.getInstance().currentRenderState;
        if (currentRenderState == null || currentRenderState.flipbookTintedParticles.isEmpty())
            return false;
