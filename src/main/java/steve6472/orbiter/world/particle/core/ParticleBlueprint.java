...
{
    private static final Logger LOGGER = Log.getLogger(ParticleBlueprint.class);
    ...
    public static void load()
    {
        ...
        Flare.getModuleManager().loadParts(OrbiterParts.PARTICLE_BLUEPRINT, Registries.PARTICLE_COMPONENT_BLUEPRINT.valueMapCodec(), (map, key) -> {
            ParticleBlueprint blueprint = new ParticleBlueprint(key, createObjects(map, key).getOrThrow());
            blueprints.put(key, blueprint);
        });
        blueprints.values().forEach(Registries.PARTICLE_BLUEPRINT::register);
    }

    private static DataResult<List<PCBlueprint<?>>> createObjects(Map<PCBlueprintEntry<?>, Object> map, Key debugKey)
    {
        ...
        if (objects.stream().noneMatch(e -> e.key().equals(ParticleComponentBlueprints.MAX_AGE.key())))
        {
            ...
            LOGGER.warning("Max age not found for particle '%s', adding default 1s".formatted(debugKey));
        }
    ...
    private static final Codec<ParticleBlueprint> INLINE_CODEC = Registries.PARTICLE_COMPONENT_BLUEPRINT.valueMapCodec().comapFlatMap(map -> {
        ParticleBlueprint blueprint = new ParticleBlueprint(Constants.key("inline"), createObjects(map, Constants.key("inline")).getOrThrow());
