 ...
 * Project: Orbiter <br>
 */
public record CreateCustomEntity(UUID uuid, List<Component> components) implements Packet<CreateCustomEntity, GameClientboundListener>
{
    public static final Key KEY = Constants.key("game/cb/create_custom_entity");
    public static final BufferCodec<ByteBuf, CreateCustomEntity> BUFFER_CODEC = BufferCodec.of(
        ...
        ExtraBufferCodecs.COMPONENT_LIST, CreateCustomEntity::components,
        CreateCustomEntity::new);

    public CreateCustomEntity(Entity entity)
    {
        this(Components.UUID.get(entity).uuid(), NetworkSerialization.selectComponents(entity, c -> !c.isAssignableFrom(UUIDComp.class)));
    }
